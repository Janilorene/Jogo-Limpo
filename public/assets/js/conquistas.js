...
async function carregarConquistas(usuarioLogadoId) {
  ...
  try {
    // Busca usuário
    const resUsuario = await fetch(`/api/usuarios/${usuarioLogadoId}`);
    if (!resUsuario.ok) throw new Error('Falha ao carregar usuário');
    const usuario = await resUsuario.json();

    // Busca progresso do usuário
    const resProgresso = await fetch(`/api/progresso?usuario_id=${usuarioLogadoId}`);
    ...
    const resConquistas = await fetch(`/api/conquistas`);
